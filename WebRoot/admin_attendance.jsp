...
function nextOrder() {
    if (orderKind == "RAND") {
    	order = 1 + Math.random() * studentNum;
    }
...
function nextStudent() {
    ...
    stuStatus = $(line).find('td:eq(1)').html();
    stuId = $(line).find('td:eq(2)').html();    
    stuName = $(line).find('td:eq(3)').html();
    curMark = $(line).find('td:last').html();
    if (stuStatus == '未修' || curMark == 'Q' || curMark == 'X') {
    	nextStudent();
    }
}
	...
	if (mark == "N") {
		...
		$.getJSON(url, function(jsonData) { 
            if(jsonData.err == 'SUCCESS') {
                ...
                else if (jsonData.msg == 'REMOVE') {
                	//$(line).attr("bgcolor", "#CCCCCC");
                	//alert($(line).attr("bgcolor"));
                	$(line).css("background-color", "#CCCCCC");
                	$(line).find('td:eq(1)').html("未修");
                }                	
                else {
                	if (jsonData.msg == 'L') {
                		jsonData.msg = '-2';
...
<form id="idFormList" name="formList" method="post" action="">
  ...
  <c:forEach items="${listAttendance}" var="pojo" varStatus="stat">
    <c:choose>
      ...
      <c:when test='${!pojo.status.equals("OK")}'>
          <tr bgcolor="#CCCCCC">
      </c:when>
      <c:otherwise>
...
<div id="idFloatingContent" class="floating_area" style="position:absolute;display:none;border:2px blue solid;background-color:#eee;width:520px;height:300px;text-align:center">
	...
	<tr><td align='center' valign='middle' width="65%">
		...
		<div style='padding:5px;'><input style='font-size:14pt' type='button' value='关闭' onClick="javascript:addMark('CLOSE');"/></div>
	...
	<tr><td colspan='2'>
	    <input style='font-size:14pt' type='button' value='未修' onClick="javascript:addMark('REMOVE');"/> 
